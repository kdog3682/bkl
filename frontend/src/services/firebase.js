...
const firebaseConfig = {
    ...
    databaseURL:
        "https://hammy-math-class-default-rtdb.firebaseio.com",
...
async function deleteDocuments(...path) {
    const docs = await readDocuments(...path)
...
async function updateDocumentField(...args) {
    const [path, field, payload] = splitOnceReverse(args, 2)
    switch (type(payload)) {
        case "Object":
            payload = payload
            break
