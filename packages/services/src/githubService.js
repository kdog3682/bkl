...
/* deno-fmt-ignore */ import { appendVariable, clip, } from "/home/kdog3682/2023/node-utils.js"
...
function parseGithubUrl(url) {
    ...
    let m 
        if (m = match(url, /github.com\/(\S+?)\/(\S+?)(?:\/?$|\/\w+)/)) {
            return m
        }
...
async function getRepoFiles(url, opts = {}) {
    const checkpoint = new Checkpoint(opts)
    ...
    const runner = async (path) => {
        ...
        if (data?.type == "file") {
            // if (!checkpoint.checkFile(file.name)) {
            // return
            // }
            if (opts.parser) {
            ...
            // if (!checkpoint.checkDir(data.name)) {
            // return
            // }
            const store = []
...
getRepoFiles('https://github.com/yorickpeterse/nvim-grey', {recursive: true, parser: namer, limit: 50, file: {include: /lua$/}}).then(console.log)
