...
async function updateDocument(...args) {
    ...
    return await updateDoc(
        docRef,
        txv(payload, firebaseTransformer),
    )
}
...
function useFirebaseStudent(studentUsername) {
    ...
    async function getAssignments() {
        const assignments = await readDocuments(
            "users",
            studentUsername,
            "assignments",
        )
...
function exampleEditAssignmentscallback(assignment, i) {
...
function useLogin() {
    ...
    const accountDataStorageKey = "accountData"
    const assignmentDataStorageKey = "assignmentDataStorageKey"
    ...
    async function login(username, password, rememberMe) {
        ...
        if (json) {
            if (toValue(password) == json.password) {
                status.value = "success.loggedInViaPassword"
                setAccount(json)
                if (rememberMe.value) {
                    storage.set(accountDataStorageKey, json, {
                        months: 2,
                    })
                } else {
                    storage.session.set(
                ...
                status.value = "error.invalidPassword"
            }
        } else {
            status.value = "error.invalidUsername"
        }
    }
    return {
        account: accountData,
...
const loginAnonymously = async (rememberMe = true) => {
    try {
        if (rememberMe === false) {
...
// console.log(loginAnonymously().then(console.log))
